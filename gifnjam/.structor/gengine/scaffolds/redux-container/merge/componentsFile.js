...
function getFile(dataObject, dependencies) {
    ...
    if (index && index.indexFilePath && index.indexSourceCode) {
        if (namespace && namespace.length > 0) {
            sourceCode = _structorCommons.gengine.injectNamespaceComponent(index.indexSourceCode, namespace, _path2.default.join('modules', namespace));
        } else {
            sourceCode = _structorCommons.gengine.injectModuleComponent(index.indexSourceCode, componentName, _path2.default.join('containers', componentName));
        }
    } else {
